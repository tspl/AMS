...
public partial class Donor_Free_Room_Allocation : System.Web.UI.Page
{
    ...
    public decimal rent, depo, tot, other, cashierliable, am, se, gt = 0, originaldepo, originalrent, newrent, newdepo, advance, netpayable;
    ...
    {        
        if (!IsPostBack)
        {
            ...
            DateTime cur = DateTime.Parse(Session["cur"].ToString());
            ViewState["action"] = "NILL";
            ViewState["auction"] = "NILL";
            ViewState["maxhour"] = "no";
            ViewState["gchk"] = "no";
            ViewState["startclub"] = "no";
            ViewState["txtroomrent"] = "0";
            ViewState["txtsecuritydeposit"] = "0";
            ViewState["hours"] = "0";
            ViewState["txtinmatecharge"] = "0";
            ViewState["txtinmatedeposit"] = "0";
            ViewState["txtgranttotal"] = "0";

            ViewState["txtothercharge"] = "0";
            ViewState["txtadvance"] = "0";
            ViewState["txtnetpayment0"] = "0";
            Session["reschkin"] = "";
            Session["reserv"] = "no";
            Session["clubbing"] = "Not";
            //Session.Timeout = 60;
            check();
            ...
            dtclub.Columns.Add("Type");
            dtclub.Columns.Add("Checkin");
            dtclub.Columns.Add("Checkout");
            dtclub.Columns.Add("No of hours");
            dtclub.Columns.Add("Passno");
            dtclub.Columns.Add("Rent");
            dtclub.Columns.Add("Deposit");
            dtclub.Columns.Add("Othercharge");
            dtclub.Columns.Add("Inmatecharge");
            dtclub.Columns.Add("Inmatedeposit");
            dtclub.Columns.Add("Reserve No");
            dtclub.Columns.Add("Building");
            dtclub.Columns.Add("Room");
            ...
            try
            {
                OdbcCommand cmdDCclub = new OdbcCommand();
                cmdDCclub.Parameters.AddWithValue("tblname", "p_type_of_user  ");
                cmdDCclub.Parameters.AddWithValue("attribute", "DISTINCT TYPE ");
               // cmdDCclub.Parameters.AddWithValue("conditionv", " ");
                DataTable dtDCclub = new DataTable();
                dtDCclub = objcls.SpDtTbl("CALL  selectdata(?,?)", cmdDCclub);

                DataRow dr = dtDCclub.NewRow();
                dr["TYPE"] = "--Select--";
                dtDCclub.Rows.InsertAt(dr,0);
                cmballoctype.DataSource = dtDCclub;
                cmballoctype.DataBind();

                //dt = DateTime.Parse(dtDCclub.Rows[0][0].ToString());
                //string dtdd = dt.ToString("yyyy/MM/dd");
                //Session["dayend"] = dtdd.ToString();
            }
            catch
            {
                okmessage("Tsunami ARMS - Warning", "Policies not provided.");
                this.ScriptManager1.SetFocus(btnOk);
                string prevpage = Request.UrlReferrer.ToString();
                Response.Redirect(prevpage);
            }
            ...
            try
            {
                ...
                else
                {
                    ...
                    DateTime getYear1 = DateTime.Parse(Session["cur"].ToString());;
           ...
           // cmdpR.Parameters.AddWithValue("conditionv"," rowstatus <> 2 ");
            ...
            //ClientScript.RegisterStartupScript("SetInitialFocus", "<script>document.getElementById('" + txtswaminame.ClientID + "').focus();</script>");
...
m_season.startdate AND m_season.enddate AND m_season.season_id='" + Session["seasonid"] + "'  GROUP BY m_season.season_id),0) AS 'Unclaimed'  FROM  t_daily_transaction,m_season  WHERE t_daily_transaction.ledger_id = '2' AND t_daily_transaction.DATE BETWEEN m_season.startdate AND m_season.enddate AND m_season.season_id='" + Session["seasonid"] + "'  GROUP BY m_season.season_id";
    ...
    #region donorfreepageload
    private void donorfreepageload()
    {
        try
        {
            //int useid = int.Parse(Session["userid"].ToString());
            Title = "Tsunami ARMS - Donor Free Room Allocation";
            ...
            donorallocpanel.Visible = true;
            clear();
            lblhead.Text = "DONOR FREE ALLOCATION";
            Session["allotype"] = "DONOR FREE ALLOCATION";
            donorallocgrid();
    ...
    public void directallocnonoccupiedroom()
    {
        try
        {
            ...
            date1 = DateTime.Now;
            rentcheckpolicy();
            if (measurement == "Hour" && lblhead.Text == "DONOR FREE ALLOCATION")
            {
            ...
            else if (measurement == "Day")
            {
                ...
                if (measurement == "Time Crossing")
                {
                    ...
                    CIN = IND + " " + INT;
                    COUT = IND + " " + minunits;
                    DataTable dt_diff = objcls.DtTbl("SELECT TIMEDIFF(STR_TO_DATE('" + COUT + "','%d/%m/%Y %l %p'),STR_TO_DATE('" + CIN + "','%d/%m/%Y %l:%i %p')),CASE  WHEN  CAST(TIMEDIFF(STR_TO_DATE('" + COUT + "','%d/%m/%Y %l %p'),STR_TO_DATE('" + CIN + "','%d/%m/%Y %l:%i %p')) AS CHAR(1)) = '-' THEN 'ok' ELSE 'not' END AS 'chk'  ");
                    ...
                    if (diff1 > 0 && (dt_diff.Rows[0][1].ToString() == "not" ))
                    {
                    ...
                    else
                    {
                        ...
                        DataTable dt_diff2 = objcls.DtTbl("SELECT TIMEDIFF(STR_TO_DATE('" + COUT1 + "','%d/%m/%Y %l:%i %p'),STR_TO_DATE('" + CIN + "','%d/%m/%Y %l:%i %p'))");
                        TimeSpan diff2 = TimeSpan.Parse(dt_diff2.Rows[0][0].ToString());
                        int diff3 = 0;
                        diff3 = Convert.ToInt32(diff2.TotalHours);
        ...
        try
        {
            ...
            //string startdate,lastdate,enddate;
                            
            //DataTable dtRRx = new DataTable();
            //DataTable dtRRvb = new DataTable();

            //if (ViewState["startclub"].ToString() == "ok")
            //{
            //    startdate = gvclub.Rows[0].Cells[1].Text;
            //    lastdate = gvclub.Rows[gvclub.Rows.Count - 1].Cells[2].Text;
            ...
            //else
            //{
            //    startdate = txtcheckindate.Text + " " + txtcheckintime.Text;
            //    lastdate = txtcheckout.Text + " " + txtcheckouttime.Text;
            //}
            //enddate = txtcheckout.Text + " " + txtcheckouttime.Text;
            //String SSq = "SELECT TIMEDIFF(STR_TO_DATE('" + enddate + "','%d/%m/%Y %l:%i %p'), STR_TO_DATE('" + startdate + "','%d/%m/%Y %l:%i %p'))";
            ...
            //cmdRRx.Parameters.AddWithValue("tblname", " m_rent ,m_room");
            //cmdRRx.Parameters.AddWithValue("attribute", " m_rent.rent,m_rent.security_deposit");
            //cmdRRx.Parameters.AddWithValue("conditionv", " ('" + tottime + "' > m_rent.start_duration)  AND ('" + tottime + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + Convert.ToInt32(cmbBuild.SelectedValue) + "'  AND  room_cat_id = m_rent.room_category  AND reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE ='" + cmballoctype.SelectedValue + "' )");
            ...
            //String SSqt = "SELECT TIMEDIFF(STR_TO_DATE('" + lastdate + "','%d/%m/%Y %l:%i %p'), STR_TO_DATE('" + startdate + "','%d/%m/%Y %l:%i %p'))";
            //DataTable DTSSqt = objcls.DtTbl(SSqt);
            //TimeSpan totperiodt = TimeSpan.Parse(DTSSqt.Rows[0][0].ToString());

            //// TimeSpan actperiod = actualvact - aldate;
            //int prevtottime = 0;
            //prevtottime = Convert.ToInt32(totperiodt.TotalHours);
            //if ((totperiodt.Minutes > 0) && (totperiodt.Minutes < 30))
            //{
            //    prevtottime++;
            //}

            //OdbcCommand cmdRRvb = new OdbcCommand();
            //cmdRRvb.Parameters.AddWithValue("tblname", " m_rent ,m_room");
            //cmdRRvb.Parameters.AddWithValue("attribute", " m_rent.rent,m_rent.security_deposit");
            //cmdRRvb.Parameters.AddWithValue("conditionv", " ('" + prevtottime + "' > m_rent.start_duration)  AND ('" + prevtottime + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  m_room.room_cat_id = m_rent.room_category  AND reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE ='" + cmballoctype.SelectedValue + "' )");
          
            //dtRRvb = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdRRvb);
            ...
            cmdR.Parameters.AddWithValue("conditionv", " ('" + hrs_used + "' >= m_rent.start_duration)  AND ('" + hrs_used + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  room_cat_id = m_rent.room_category AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "')  ");
            DataTable dtR = new DataTable();
            dtR = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdR);
            if (dtR.Rows.Count > 0)
            {
                //    txtsecuritydeposit.Text = dtR.Rows[0]["security"].ToString(); haneesh_new
                if (cmballoctype.SelectedValue == "Donor Free Allocation")
                {
                    txtroomrent.Text = "0";
                ...
                else
                {
                    ...
                    //else
                    //{
                    //    txtsecuritydeposit.Text = (Convert.ToInt32(dtRRx.Rows[0]["security_deposit"].ToString()) - Convert.ToInt32(dtRRvb.Rows[0]["security_deposit"].ToString())).ToString();
                    //}
                ...
                txttotalamount.Text = (Convert.ToInt32(dtR.Rows[0]["security_deposit"].ToString()) + Convert.ToInt32(txtroomrent.Text)).ToString();
                txtadvance.Text = "0";
                txtothercharge.Text = "0";
                txtnetpayment0.Text = txttotalamount.Text;
            ...
            if (dtS.Rows.Count > 0)
            {
                ...
                if (dtAPS.Rows.Count > 0)
                {
                    ...
                    for (int ii = 0; ii < dtAPS.Rows.Count; ii++)
                    {
                        ...
                        cmbAP.Parameters.AddWithValue("conditionv", "mes.service_unit_id=t_policy_allocation.is_multi_room and alloc_policy_id=" + sid + " and reqtype='"+cmballoctype.SelectedValue+"' and curdate() between fromdate and todate and t_policy_allocation.rowstatus<>" + 2 + "");
    ...
    public void generalallocationbuilding()
    {
        try
        {
            ...
            //{
                ...
                cmdB.Parameters.AddWithValue("conditionv", "room.build_id=build.build_id  order by build.buildingname asc");
            ...
            //    cmdB.Parameters.AddWithValue("tblname", "m_sub_building as build,m_room as room");
            //    cmdB.Parameters.AddWithValue("attribute", "distinct build.buildingname,build.build_id");
            //    cmdB.Parameters.AddWithValue("conditionv", "room.build_id=build.build_id  and room.rowstatus<>" + 2 + "  and room.housekeepstatus=" + 1 + " order by build.buildingname asc");
            //    DataTable dtB = new DataTable();
            //    dtB = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdB);
            //    DataRow row = dtB.NewRow();
            //    row["build_id"] = "-1";
    ...
    public void printgeneral()
    {
        try
        {
            ...
            if (Session["reserv"].ToString() == "ok")
            {
                if (Session["clubbing"].ToString() != "ok")
                {
                ...
                else
                {
                    ...
                    rent = rent - (Convert.ToDecimal(txtadvance.Text) - Convert.ToDecimal(txtothercharge.Text));
                    depo = Convert.ToDecimal(txtsecuritydeposit.Text.ToString());
                }
            }
            ...
            for (int i = 1; i < 25; i++)
            {
                ...
                else if (i == 6)
                {
                    ...
                    PdfPCell cell27a = new PdfPCell(new Phrase(new Chunk("Total :" + totalsum.ToString(), font12)));
                    cell27a.Border = 0;
                    cell27a.Colspan = 2;
                ...
                else if (i == 20)
                {
                    ...
                    else if (Session["clubbing"].ToString() == "ok")
                    {
                        PdfPCell cell9821 = new PdfPCell(new Phrase("We agree to accommodate devotees claiming this room with donor pass during our stay", font10));
                        cell9821.Border = 0;
                        cell9821.Colspan = 14;
                        cell9821.FixedHeight = 15;
                        table.AddCell(cell9821);
    ...
    public void gridviewnoofinmates()
    {
        try
        {
            ...
            if (cmbRooms.SelectedValue != "-1" && txtcheckout.Text != "" && txtcheckouttime.Text != "" && txtnoofinmates.Text != "")
            {
                ...
                string stmx = @"SELECT noofinmates,maxinmates,rate FROM m_inmate WHERE room_id = '" + cmbRooms.SelectedValue + "' AND '" + hrs_used + "'  > start_duration AND '" + hrs_used + "' <= end_duration  ";
                ...
                if (dt_stmx.Rows.Count > 0)
                {
                    ...
                    if (Convert.ToInt32(txtnoofinmates.Text) > Convert.ToInt32(dt_stmx.Rows[0][0].ToString()))
                    {
                        ...
                        if (Convert.ToInt32(txtnoofinmates.Text) <= Convert.ToInt32(dt_stmx.Rows[0][1].ToString()))
                        {
                            ...
                            txtgranttotal.Text = (Convert.ToInt32(txtinmatecharge.Text) + Convert.ToInt32(txtnetpayment0.Text)).ToString();
        ...
        try
        {
            ...
            if (rrmm == "clear")
            {
                ...
                else
                {
                    ...
                    if (Session["Clubbing"] == "ok" && (cmballoctype.SelectedValue != "Donor Free Allocation" || cmballoctype.SelectedValue != "Donor Paid Allocation"))
                    {
                        int t = 0;
                        ...
                        cmdBP1.Parameters.AddWithValue("conditionv", " (CURDATE() BETWEEN fromdate AND todate ) AND reqtype = '"+ cmballoctype.SelectedValue +"'");
    ...
    public void roomrentcalculate()
    {
        try
        {
           ...
           // rentcheckpolicy();
            if (cmballoctype.SelectedValue == "Donor Free Allocation" || cmballoctype.SelectedValue == "Donor Paid Allocation")
            {
             ...
             if (ViewState["startclub"].ToString() != "ok")
              {
                ...
                if (measurement == "Hour")
                {
                    minunit = int.Parse(minunits.ToString());
                    string checkin = txtcheckindate.Text + " " + txtcheckintime.Text;
                    DataTable dt_out = objcls.DtTbl("SELECT DATE_FORMAT(DATE_ADD(STR_TO_DATE('" + checkin + "','%d/%m/%Y %l:%i %p'), INTERVAL " + minunit + " HOUR ),'%d/%m/%Y'),DATE_FORMAT(DATE_ADD(STR_TO_DATE('" + checkin + "','%d/%m/%Y %l:%i %p'), INTERVAL " + minunit + " HOUR ),'%l:%i %p')");
                    txtcheckout.Text = dt_out.Rows[0][0].ToString();
                    txtcheckouttime.Text = dt_out.Rows[0][1].ToString();
                    txtnoofdays.Text = minunit.ToString();
                    tt = 1;
            ...
            else //if (lblhead.Text == "GENERAL ALLOCATION")
            {
                double oldrent;
                ...
                string chkin = objcls.yearmonthdate(txtcheckindate.Text) + " " + txtcheckintime.Text;

               
                DataTable dt_nw = objcls.DtTbl("select '" + chkin + "' as 'dt',date_format(now(),'%r') as 'time',now() as 'NW'");

                date1 = DateTime.Parse(dt_nw.Rows[0]["dt"].ToString());
                defhour = Convert.ToInt32(Session["defhour"].ToString());

                tyme = tyme + defhour;
                if (tyme > maxhour)
                {
                    okmessage("Tsunami ARMS - Warning", "Genral allocation maximum time reached----");
               ...
               //     //string vacatx = txtPropCheckOut.Text;
               //     string enddate = txtcheckout.Text + " " + txtcheckouttime.Text;
            ...
            defhour = Convert.ToInt32(txtnoofdays.Text);
            if (defhour > 0)
            {
                ...
                cmdRR.Parameters.AddWithValue("conditionv", " ('" + defhour + "' > m_rent.start_duration)  AND ('" + defhour + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  m_room.room_cat_id = m_rent.room_category AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "') "); // AND reservation_type =  (SELECT id FROM p_type_of_user WHERE TYPE ='" + cmballoctype.SelectedValue + "' )");
    ...
    protected void btnallocate_Click(object sender, EventArgs e)
    {
        ...
        if (ViewState["startclub"].ToString() == "ok")
        {
            string startdate = gvclub.Rows[0].Cells[1].Text;
            string lastdate = gvclub.Rows[gvclub.Rows.Count - 1].Cells[2].Text;



            string[] checkoutSplit = lastdate.Split(' ');
            string[] checkinSplit = startdate.Split(' ');

            txtcheckindate.Text = checkinSplit[0].ToString();
            txtcheckintime.Text = checkinSplit[1].ToString() + " " + checkinSplit[2].ToString();

            txtcheckout.Text = checkoutSplit[0].ToString();

            txtcheckouttime.Text = checkoutSplit[1].ToString() + " " + checkoutSplit[2].ToString();
        ...
        try
        {
            ...
            string difff = "SELECT TIMEDIFF(STR_TO_DATE('" + COUT + "','%d/%m/%Y %l:%i %p'),STR_TO_DATE('" + IND + "','%d/%m/%Y %l:%i %p'))";
            DataTable dt_diff = objcls.DtTbl(difff);
            TimeSpan diff = TimeSpan.Parse(dt_diff.Rows[0][0].ToString());
            int diff1 = 0;
            diff1 = Convert.ToInt32(diff.TotalHours);
            if ((diff.Minutes > 0) && (diff.Minutes < 30))
    ...
    protected void btnclear_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            btncancel.Text = "View Alloc";
            gdalloc.Visible = false;
            chkclubbing.Checked = false;
            gvclub.Visible = false;
            cmballoctype.SelectedValue = "--Select--";
    ...
    protected void cmbaltbulilding_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            string extra = @"select extra_billing from p_alter_room_allocation where season_id=" + int.Parse(Session["season"].ToString()) + " and curdate() between from_date and to_date and type_of_allocation=(SELECT id FROM p_type_of_user WHERE TYPE ='"+cmballoctype.SelectedValue+"' ) and row_status <>2";
            ...
            if (Session["clubbing"].ToString() == "ok")
            {
                ren = Convert.ToInt32(Session["roomrent"].ToString());
            }
            ...
            {
                qry = "  GROUP BY mr.room_id";  //and mr.housekeepstatus=" + 1 + "
            }
            ...
            if (int.Parse(dt_ex.Rows[0][0].ToString()) == 1)
            {
                ...
                cmdDis.Parameters.AddWithValue("conditionv", "mr.build_id=msb.build_id AND mr.roomstatus='1'  AND  mr.rowstatus!='2' AND rc.room_cat_id=mr.room_cat_id AND rt.room_category = mr.room_cat_id  AND ( '" + alttime + "' > rt.start_duration)  AND ('" + alttime + "' <= rt.end_duration ) AND  rt.rent >= " + ren + "  AND rt.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "')  AND mr.build_id='" + Convert.ToInt32(cmbaltbulilding.SelectedValue) + "' " + qry);
        ...
        else
        {
            try
            {
                ...
                else
                {
                    ...
                    cmdR.Parameters.AddWithValue("tblname", " m_rent ,m_room");
                    cmdR.Parameters.AddWithValue("attribute", " m_rent.rent,m_rent.security_deposit");
                    cmdR.Parameters.AddWithValue("conditionv", " ('" + alttime + "' >= m_rent.start_duration)  AND ('" + alttime + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  room_cat_id = m_rent.room_category  AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "') ");
                    DataTable dtR = new DataTable();
                    dtR = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdR);
                    if (int.Parse(txtsecuritydeposit.Text) > int.Parse(dtR.Rows[0]["security_deposit"].ToString()))
                    {
                    ...
                    Session["roomrent"] = dtR.Rows[0]["rent"].ToString();
                    rent = decimal.Parse(dtR.Rows[0]["rent"].ToString());
                    // rent = tt * rent;
                    depo = decimal.Parse(txtsecuritydeposit.Text.ToString());
                    ...
                    if (cmballoctype.SelectedValue != "Donor Free Allocation")
                    {
                        ...
                        tot = rent + depo + Convert.ToDecimal(txtothercharge.Text);
                        txttotalamount.Text = tot.ToString();
                      //  txttotalamount.Text = (rent+depo).ToString();
                        netpayable = tot - advance;
                        txtnetpayment0.Text = netpayable.ToString();
                    }
                    else
                    {
                        rent = 0;
                    ...
                    else
                    {
                        txtinmatecharge.Text = "0";
                        txtinmatedeposit.Text = "0";
                        txtgranttotal.Text = txtnetpayment0.Text;
                    }
                }
            }
            catch
        ...
        if (txtdonorpass.Text != "")
        {
            try
            {
                ...
                if (Session["clubbing"].ToString() == "ok")
                {
                    ...
                    if (ViewState["startclub"].ToString() == "ok")
                    {
                        for (int i = 0; i < gvclub.Rows.Count; i++)
                        {
                            if (txtdonorpass.Text == gvclub.Rows[i].Cells[4].Text)
                            {
                                okmessage("Tsunami ARMS - Warning", "Pass already Entered----");
                                txtdonorpass.Text = "";
                                return;
                            }
                        }
                    }
                    if (cmballoctype.SelectedValue == "--Select--")
                    {
                        okmessage("Tsunami ARMS - Warning", "Pleaase select the type of allocation");
                        txtdonorpass.Text = "";
                        return;
                    }
                    else if (cmballoctype.SelectedValue == "Donor Free Allocation")
                    {
                        str = " and passtype='0'";
                    }
                    else if (cmballoctype.SelectedValue == "Donor Paid Allocation")
                    {
                        str = " and passtype='1'";
                    }
                }
                ...
                {
                    str = " and passtype='0'";
                    cmballoctype.SelectedValue = "Donor Free Allocation";
                }
                ...
                cmdP.Parameters.AddWithValue("conditionv", "passno= " + int.Parse(txtdonorpass.Text) + " and pass.donor_id=don.donor_id " + str);
                DataTable dtaP = new DataTable();
                dtaP = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdP);
                if (dtaP.Rows.Count > 0)
                {
                    string rmid = dtaP.Rows[0]["room_id"].ToString();

                    if (dtaP.Rows[0]["passtype"].ToString() == "0")
                    {

                        if (Session["clubbing"].ToString() == "ok")
                        {
                            OdbcCommand cmdPclub = new OdbcCommand();
                            cmdPclub.Parameters.AddWithValue("tblname", "p_clubbing  INNER JOIN p_type_of_user ON p_type_of_user.id = p_clubbing.reserve_types");
                            cmdPclub.Parameters.AddWithValue("attribute", "DISTINCT TYPE");
                            cmdPclub.Parameters.AddWithValue("conditionv", " CURDATE() BETWEEN from_date AND to_date AND  clubbing_status= 1 AND TYPE='Donor Free Allocation'");
                            DataTable dtaPclub = new DataTable();
                            dtaPclub = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdPclub);
                            if (dtaPclub.Rows.Count > 0)
                            {

                            }
                            else
                            {
                                okmessage("Tsunami ARMS - Warning", "This policy is not clubbed ");
                                txtdonorpass.Text = "";
                                return;
                            }
                        }
                    ...
                    else
                    {
                        if (Session["clubbing"].ToString() == "ok")
                        {
                            ...
                            cmdPclub.Parameters.AddWithValue("tblname", "p_clubbing  INNER JOIN p_type_of_user ON p_type_of_user.id = p_clubbing.reserve_types");
                            cmdPclub.Parameters.AddWithValue("attribute", "DISTINCT TYPE");
                            cmdPclub.Parameters.AddWithValue("conditionv", " CURDATE() BETWEEN from_date AND to_date AND  clubbing_status= 1 AND TYPE='Donor Paid Allocation'");
                            DataTable dtaPclub = new DataTable();
                            dtaPclub = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdPclub);
                            if (dtaPclub.Rows.Count > 0)
                            {
                    ...
                    {
                        string pno = "",ptyp="";
                        for (int n = 0; n < gvclub.Rows.Count; n++)
                        {
                            if (gvclub.Rows[n].Cells[0].Text == "Donor Paid Allocation" )
                            {
                                pno = gvclub.Rows[n].Cells[4].Text;
                                ptyp = "1";
                            }
                            if (gvclub.Rows[n].Cells[0].Text == "Donor Free Allocation")
                            {
                                pno = gvclub.Rows[n].Cells[4].Text;
                                ptyp = "0";
                            }
                        ...
                        //SELECT room_id FROM t_donorpass WHERE passno =3449


                               OdbcCommand cmdPqwer = new OdbcCommand();
                               cmdPqwer.Parameters.AddWithValue("tblname", "t_donorpass");
                               cmdPqwer.Parameters.AddWithValue("attribute", "room_id");
                               cmdPqwer.Parameters.AddWithValue("conditionv", "passno= " + pno + " AND  passtype=  " + ptyp + "");
                               DataTable dtaPqwer = new DataTable();
                               dtaPqwer = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdPqwer);
                               if (dtaPqwer.Rows.Count > 0)
                               {
                                   if (rmid != dtaPqwer.Rows[0][0].ToString())
                                   {
                                       okmessage("Tsunami ARMS - Warning", "This pass is for different room-----");
                                         return;
                                   }
                    ...
                    string st = @"SELECT reserve_no,DATE_FORMAT(reservedate,'%d-%m-%Y') as 'resdate' FROM t_roomreservation WHERE pass_id = '" + dtaP.Rows[0]["pass_id"].ToString() + "'  AND  t_roomreservation.status_reserve=0  ";
                    DataTable dt_st = objcls.DtTbl(st);
                    if (dt_st.Rows.Count > 0)
                    {
                        //if (Session["clubbing"].ToString() == "ok")
                        //{
                        //    okmessage("Tsunami ARMS - Warning", "This pass is reserved on " + dt_st.Rows[0]["resdate"].ToString() + "-----");
                        //    return;
                        //}
                    ...
                    if (currentyear == passyear)
                    {
                        ...
                        if (curseason == passeason)
                        {
                            ...
                            {  
                                ////--------------------->changed by jithu based on room reserved
                            ...
                            else if (dtaP.Rows[0]["status_pass_use"].Equals("1"))
                            {
                                ...
                                cmbBuild.SelectedValue = dtaP.Rows[0]["build_id"].ToString();                         
                                cmbRooms.SelectedValue = dtaP.Rows[0]["room_id"].ToString();
                                //  donorreservealloc(); want check it..........
                                donordirectalloc();
                                donorallocpassselectedgrid();
                                if (Session["clubbing"].ToString() == "ok")
                                {
    ...
    public void donordirectalloc()
    {
        try
        {
            ...
            cmdDDA.Parameters.AddWithValue("attribute", "room_id,roomstatus,rowstatus");
            cmdDDA.Parameters.AddWithValue("conditionv", "build_id=" + int.Parse(cmbBuild.SelectedValue.ToString()) + " and room_id=" + int.Parse(cmbRooms.SelectedValue.ToString()) + " ");
            DataTable dtDDA = new DataTable();
            dtDDA = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdDDA);

            if (dtDDA.Rows.Count > 0)
            {
                if (dtDDA.Rows[0]["rowstatus"].ToString() != "2")
                {
                ...
                else if (dtDDA.Rows[0]["rowstatus"].ToString() == "2")
                {
                    directallocnonoccupiedroom();
                    SeasonEndCheck();
                    //alternate room
                    lblMsg.Text = "Room deleted..Select alternate room?";
                    ViewState["action"] = "alt_room_donor";
                    pnlOk.Visible = false;
                    pnlYesNo.Visible = true;
                    ModalPopupExtender1.Show();
        ...
        try
        {
            ...
            else
            {
                ...
                if (txtcheckout.Text != "")
                {
                    ...
                    string chkin = txtcheckindate.Text + " " + txtcheckintime.Text;

                    String SS = "SELECT TIMEDIFF(STR_TO_DATE('" + odate + "','%d/%m/%Y %l:%i %p'), STR_TO_DATE('" + chkin + "','%d/%m/%Y %l:%i %p'))";
                    DataTable DTSS = objcls.DtTbl(SS);
                    TimeSpan actperiod = TimeSpan.Parse(DTSS.Rows[0][0].ToString());
                    int hrs_used = 0;
                    hrs_used = Convert.ToInt32(actperiod.TotalHours);
    ...
    public void rentcalculation()
    {
        ...
        cmdR.Parameters.AddWithValue("tblname", " m_rent ,m_room");
        cmdR.Parameters.AddWithValue("attribute", " m_rent.rent,m_rent.security_deposit");
        cmdR.Parameters.AddWithValue("conditionv", " ('" + dd + "' >= m_rent.start_duration)  AND ('" + dd + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  room_cat_id = m_rent.room_category  AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '"+cmballoctype.SelectedValue+"') ");
        DataTable dtR = new DataTable();
        dtR = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdR);
        if (dtR.Rows.Count > 0)
        {
            ...
            {
                txtnetpayment0.Text = dtR.Rows[0]["security_deposit"].ToString(); 
            ...
            try
            {
                ...
                if (cmballoctype.SelectedValue == "Donor Free Allocation")
                {
                    ...
                    if (measurement == "Time Crossing")
                    {
                        ...
                        if (diff1 > 0 && (dt_diff.Rows[0][1].ToString() == "not"))
                        {
                            DataTable dt_out = objcls.DtTbl("SELECT STR_TO_DATE('" + COUT + "','%d/%m/%Y %l %p')");
                            out1 = DateTime.Parse(dt_out.Rows[0][0].ToString());

                        }
                        else
                        {
                            DataTable dt_out = objcls.DtTbl("SELECT STR_TO_DATE(DATE_ADD('" + COUT + "',INTERVAL 1 DAY),'%d/%m/%Y %l %p')");
                        ...
                        string strzzz = objcls.yearmonthdate(txtcheckout.Text) + " " + txtcheckouttime.Text;
                        // DateTime out1 = DateTime.Parse(COUT1);
                        DateTime out2 = DateTime.Parse(strzzz);
                ...
                if (dtAPS.Rows.Count > 0)
                {
                    ...
                    for (int ii = 0; ii < dtAPS.Rows.Count; ii++)
                    {
                        ...
                        if (ViewState["startclub"].ToString() == "ok")
                        {
                            gggg = cmballoctype.SelectedValue;
                ...
                if (ViewState["startclub"].ToString() == "ok" && (cmballoctype.SelectedValue != "Donor Free Allocation" && cmballoctype.SelectedValue != "Donor Paid Allocation"))
                {
                    tyme = 0;
                    for (int i = 0; i < gvclub.Rows.Count; i++)
                    {
                        if (gvclub.Rows[i].Cells[0].Text == cmballoctype.SelectedValue)
                        {

                            tyme = Convert.ToInt32(gvclub.Rows[i].Cells[3].Text) + tyme;
        ...
        if (txtcheckout.Text != "" && txtcheckouttime.Text != "")
        {
            if (cmballoctype.SelectedValue == "Donor Free Allocation")
            {
                ...
                if (measurement == "Time Crossing")
                {
                    ...
                    {
                        DataTable dt_out = objcls.DtTbl("SELECT CAST(CONCAT(DATE_ADD( STR_TO_DATE('" + IND + "','%d/%m/%Y'),INTERVAL 1 DAY),' ','3 PM') AS CHAR(20)) AS 'time' ");
                        out1 = DateTime.Parse(dt_out.Rows[0][0].ToString());
                    }
                    string strzzz = objcls.yearmonthdate(txtcheckout.Text) + " " + txtcheckouttime.Text;
                    // DateTime out1 = DateTime.Parse(COUT1);
    ...
    public void AllocationSave()
    {
        ...
        try
        {
            ...
            //if (Session["inmate"] == "ok")
            //{
            //    string stvc = @"INSERT INTO t_inmateallocation (alloc_id,extra_inmates,TIME,rate,totalcharge) VALUES ('" + id + "','" + Session["count"].ToString() + "','" + txthours.Text + "','" + Session["inmrate"].ToString() + "','" + txtinmatecharge.Text + "')";
    ...
    protected void btnYes_Click(object sender, EventArgs e)
    {
        if (ViewState["action"].ToString() == "Allocate")
        {
            ...
            try
            {
                ...
                if (donorgrid.Visible == true)
                #region not using on 13-11-2013
                {
                    ...
                    try
                    {
                        ...
                        barencrypt = base64Encode(barencrypt.ToString());
                        // #endregion
                        DataTable dt_up = objcls.DtTbl("select date_format(now(),'%Y/%m/%d %T')");
                        //DateTime update = DateTime.Parse(Session["cur"].ToString()); ;
                        string updatedate = dt_up.Rows[0][0].ToString();
                        ...
                        // #endregion
                        DataTable dt_cur = objcls.DtTbl("select date_format(now(),'%Y/%m/%d %T')");
                        ...
                        INT = txtcheckintime.Text.ToString();
                        CIN = IND + " " + INT;
                        //DataTable dtf = objcls.DtTbl("select date_format(str_to_date('" + CIN + "','%d/%m/%Y %l:%i %p'),'%Y/%m/%d %T')");
                        DateTime cinn = DateTime.Parse(CIN);
                        CIN = cinn.ToString("yyyy/MM/dd") + " " + cinn.ToString("HH:mm:ss");
                        //CIN = dtf.Rows[0][0].ToString();

                        OUTD = txtcheckout.Text.ToString();
                        OUTT = txtcheckouttime.Text.ToString();
                        COUT = OUTD + " " + OUTT;
                        //DataTable dtf1 = objcls.DtTbl("select date_format(str_to_date('" + COUT + "','%d/%m/%Y %l:%i %p'),'%Y/%m/%d %T')");
                        DateTime coutt = DateTime.Parse(COUT);
                        COUT = coutt.ToString("yyyy/MM/dd") + " " + coutt.ToString("HH:mm:ss");
                        //COUT = dtf1.Rows[0][0].ToString();
                        ...
                        if ((cmbState.SelectedValue == "-1") && (cmbDists.SelectedValue == "-1"))
                        {
                            ...
                            strSave = "" + id + ","
                                          ...
                                          + "" + decimal.Parse(txtroomrent.Text) + ","
                                          + "'" + "2" + "',"
                                          + "" + decimal.Parse(txtnetpayment0.Text) + ","
                                          + "" + decimal.Parse(txtsecuritydeposit.Text) + ","
                                          + "" + 0 + ","
                                          + "'" + txtreson.Text + "',"
                                          + "" + decimal.Parse(txtothercharge.Text) + ","
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            if (ViewState["startclub"].ToString() != "ok")
                            {
                                string spas = Session["passid"].ToString();
                                OdbcCommand cmd126 = new OdbcCommand("call updatedata(?,?,?)", con);
                                cmd126.CommandType = CommandType.StoredProcedure;
                                cmd126.Parameters.AddWithValue("tablename", "t_roomreservation");
                                cmd126.Parameters.AddWithValue("valu", "status_reserve=" + 2 + "");
                        ...
                        if (ViewState["startclub"].ToString() == "ok")
                        {
                            ...
                            for (int i = 0; i < gvclub.Rows.Count; i++)
                            {
                                if (gvclub.Rows[i].Cells[4].Text != "0")
                                {
                                    
                                    
                                    if (gvclub.Rows[i].Cells[0].Text == "Donor Free Allocation")
                                    {
                                        ptyp = "0";
                                    }
                                    else if (gvclub.Rows[i].Cells[0].Text ==  "Donor Paid Allocation")
                                    {
                                        ptyp = "1";
                                    }
                                    ...
                                    cmd26.Parameters.AddWithValue("convariable", "passno=" + int.Parse(gvclub.Rows[i].Cells[4].Text) + " and passtype='" + ptyp + "'");
                                    cmd26.Transaction = odbTrans;
                                    cmd26.ExecuteNonQuery();
                                    ...
                                    cmd53.Parameters.AddWithValue("tblname", "t_donorpass as pass,m_donor as don");
                                    cmd53.Parameters.AddWithValue("attribute", "don.donor_name,don.donor_id,pass.passtype,pass.pass_id");
                                    cmd53.Parameters.AddWithValue("conditionv", "passno=" + int.Parse(gvclub.Rows[i].Cells[4].Text) + " and pass.donor_id=don.donor_id and passtype='" +ptyp + "'");
                                    ...
                                    Session["donorid"] = dtt53.Rows[0]["donor_id"].ToString();
                                   // Session["passid"] = dtt53.Rows[0]["pass_id"].ToString();
                                    Session["passid"] =(0).ToString();  // bcz if multiple pass is used, no specific pass id can b used
                                    pass = dtt53.Rows[0]["passtype"].ToString();
                                ...
                                OdbcCommand cmdxbnm = new OdbcCommand("SELECT CAST(STR_TO_DATE('" + gvclub.Rows[i].Cells[1].Text + "','%d/%m/%Y %l:%i %p') AS CHAR(25)) as 'dt1',CAST(STR_TO_DATE('" + gvclub.Rows[i].Cells[2].Text + "','%d/%m/%Y %l:%i %p') AS CHAR(25)) as 'dt2'", con);
                                cmdxbnm.Transaction = odbTrans;

                                OdbcDataAdapter dacnt53vcb = new OdbcDataAdapter(cmdxbnm);
                                DataTable dttdate = new DataTable();
                                dacnt53vcb.Fill(dttdate);

                                string rsid = "null";
                                if (gvclub.Rows[i].Cells[10].Text != "0")
                                {
                                    OdbcCommand cmdxbnmbn = new OdbcCommand("SELECT reserve_id FROM t_roomreservation WHERE reserve_no='" + gvclub.Rows[i].Cells[10].Text + "'", con);
                                    cmdxbnmbn.Transaction = odbTrans;

                                    OdbcDataAdapter dacnt53vcbbn = new OdbcDataAdapter(cmdxbnmbn);
                                    DataTable dttdatebn = new DataTable();
                                    dacnt53vcbbn.Fill(dttdatebn);
                                    rsid = dttdatebn.Rows[0][0].ToString();
                                }

                                //(alloc_id,passno,checkindate,checkoutdate,rent,deposit,othercharge,alloc_type)CHKOUT1
                                clubsave = "'" + id + "','" + gvclub.Rows[i].Cells[4].Text + "','" + dttdate.Rows[0][0].ToString() + "','" + dttdate.Rows[0][1].ToString() + "','" + gvclub.Rows[i].Cells[5].Text + "','" + gvclub.Rows[i].Cells[6].Text + "','" + gvclub.Rows[i].Cells[7].Text + "','" + gvclub.Rows[i].Cells[0].Text + "'," + rsid + "";
                                OdbcCommand cmd5xcv = new OdbcCommand("CALL savedata(?,?)", con);
                        ...
                        else
                        {
                            ...
                            cmd26.Parameters.AddWithValue("tablename", "t_donorpass");
                            cmd26.Parameters.AddWithValue("valu", "status_pass_use='" + "2" + "'");
                            cmd26.Parameters.AddWithValue("convariable", "passno=" + int.Parse(txtdonorpass.Text.ToString()) + " and passtype='" + 0 + "'");
                            cmd26.Transaction = odbTrans;
                            cmd26.ExecuteNonQuery();
                            ...
                            cmd53.Parameters.AddWithValue("tblname", "t_donorpass as pass,m_donor as don");
                            cmd53.Parameters.AddWithValue("attribute", "don.donor_name,don.donor_id,pass.passtype,pass.pass_id");
                            cmd53.Parameters.AddWithValue("conditionv", "passno=" + int.Parse(txtdonorpass.Text) + " and pass.donor_id=don.donor_id and pass.passtype='" + 0 + "'");
                        ...
                        //  Session["isdepo"] = 0;
                        int isrent = 0, isdeposit = 0;
                        string reservepolicy = "SELECT is_rent,is_deposit FROM t_policy_reservation WHERE res_type='Donor Free' AND '" + curYear.ToString("yyyy-MM-dd") + "'  BETWEEN res_from AND res_to";
                            ...
                            else
                            {
                                ...
                                for (int i = 0; i < gvclub.Rows.Count; i++)
                                {
                                    if (gvclub.Rows[i].Cells[10].Text != "0")
                                    {
                                        ...
                                        string stxcv = @"SELECT t_roomreservation.reserve_id,room_rent,security_deposit FROM t_roomreservation INNER JOIN t_roomreservation_generaltdbtemp ON
 t_roomreservation_generaltdbtemp.reserve_no = t_roomreservation.reserve_no WHERE t_roomreservation.reserve_no='" + gvclub.Rows[i].Cells[10].Text + "'";
                                        DataTable dtstxcv = objcls.DtTbl(stxcv);
                                        if (dtstxcv.Rows.Count > 0)
                                        {
                                            string altype = "";
                                            if (gvclub.Rows[i].Cells[0].Text == "Donor Free Allocation")
                                            {
                                            ...
                                            else if (gvclub.Rows[i].Cells[0].Text == "General Allocation")
                                            {
                                                altype = "General";
                                            }
                                            reservepolicy = "SELECT is_rent,is_deposit FROM t_policy_reservation WHERE res_type='" + altype + "' AND '" + curYear.ToString("yyyy-MM-dd") + "'  BETWEEN res_from AND res_to";
                                            ...
                                            if (isrent == 1)
                                            {
                                                totrnt = totrnt + Convert.ToDecimal(dtstxcv.Rows[0]["room_rent"].ToString());
                                             
                                            }
                                            if (isdeposit == 1)
                                            {
                                                totdep = totdep + Convert.ToDecimal(dtstxcv.Rows[0]["security_deposit"].ToString());
                                            }
                                            ...
                                            cmd267.Parameters.AddWithValue("tablename", "t_roomreservation_generaltdbtemp");
                                            cmd267.Parameters.AddWithValue("valu", "status_reserve=" + 2 + " ");
                                            cmd267.Parameters.AddWithValue("convariable", "reserve_no = '" + gvclub.Rows[i].Cells[10].Text + "'");
                                            cmd267.Transaction = odbTrans;
                                            cmd267.ExecuteNonQuery();
                                          
                                            OdbcCommand cmd126xx = new OdbcCommand("call updatedata(?,?,?)", con);
                                            cmd126xx.CommandType = CommandType.StoredProcedure;
                                            cmd126xx.Parameters.AddWithValue("tablename", "t_roomreservation");
                                            cmd126xx.Parameters.AddWithValue("valu", "status_reserve=" + 2 + "");
                                            cmd126xx.Parameters.AddWithValue("convariable", "reserve_no = '" + gvclub.Rows[i].Cells[10].Text + "'");
                                            cmd126xx.Transaction = odbTrans;
                                            cmd126xx.ExecuteNonQuery();
    ...
    protected void txtdonortype_TextChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            cmdP.Parameters.AddWithValue("conditionv", "barcodeno = '" + txtdonortype.Text + "' and pass.donor_id=don.donor_id " + str);
            DataTable dtaP = new DataTable();
            dtaP = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdP);
            if (dtaP.Rows.Count > 0)
            {
    ...
    protected void txtReserveNo_TextChanged(object sender, EventArgs e)
    {
        if (txtReserveNo.Text != "")
        {
            ...
            if (dtreserve.Rows.Count > 0)
            {
                ...
                txtnetpayment0.Text = dtreserve.Rows[0]["balance_amount"].ToString();
                ...
                string maxi = @"SELECT max_allocdays FROM t_policy_allocation WHERE reqtype='Donor Free Allocation' AND CURDATE() BETWEEN fromdate AND todate";
                ...
                if (hour > max_check)
                {
                    ...
                    string tt = dtreserve.Rows[0]["chkin_dup"].ToString();
                    string[] splitdatefrom = (dtreserve.Rows[0]["chkin_dup"].ToString()).Split(' ');
                    string ttdate = splitdatefrom[0].ToString();
                    string tttime = splitdatefrom[1].ToString() + " " + splitdatefrom[2].ToString();
                    //string tchk = chkdate + " " + chktime;


                    string st = @"SELECT  CASE  WHEN STR_TO_DATE('" + tt + "','%Y/%m/%d %l:%i:%s %p') < STR_TO_DATE(CONCAT('" + ttdate + "',' 3:00 pm'),'%Y/%m/%d %l:%i %p') THEN CONCAT(DATE_FORMAT('" + ttdate + "','%d-%m-%Y'),' 3:00 pm') ELSE CONCAT(DATE_FORMAT(ADDDATE('" + ttdate + "',INTERVAL 1 DAY),'%d-%m-%Y'),' 3:00 pm') END AS 'chk'";
                        DataTable dt_st = objcls.DtTbl(st);
                        if (dt_st.Rows.Count > 0)
                        {
                            ...
                            string to = txtcheckout.Text + " " + txtcheckouttime.Text;

                            //

                            string stx = @"SELECT  CASE  WHEN STR_TO_DATE('" + to + "','%d-%m-%Y %l:%i %p') <= STR_TO_DATE('" + tchk + "','%d-%m-%Y %l:%i %p') THEN 'ok' ELSE 'not' END AS 'chk', TIMEDIFF(STR_TO_DATE('" + tchk + "','%d-%m-%Y %l:%i %p'),STR_TO_DATE('" + to + "','%d-%m-%Y %l:%i %p'))  ,DATE_FORMAT(STR_TO_DATE('" + chkdate + "','%d-%m-%Y'),'%d/%m/%Y') ";
                            DataTable dt_stx = objcls.DtTbl(stx);
                            if (dt_stx.Rows.Count > 0)
                            {
                                chkdate = dt_stx.Rows[0][2].ToString();
                                if (dt_stx.Rows[0][0].ToString() == "ok")
                                {
                                    ...
                                    {
                                        to = tchk;
                                        txtcheckout.Text = chkdate;
                                        txtcheckouttime.Text = chktime;
                            ...
                            string st1 = @"SELECT TIMEDIFF(STR_TO_DATE('" + to + "','%d-%m-%Y %l:%i %p'),STR_TO_DATE('" + tt + "','%Y/%m/%d %l:%i:%s %p')) ";
...
FROM m_rent,
  m_room
WHERE (" + noofhours + " > m_rent.start_duration) AND (" + noofhours + " <= m_rent.end_duration) AND m_room.room_id = " + dtreserve.Rows[0]["room_id"].ToString() + " AND m_room.build_id = " + dtreserve.Rows[0]["build_id"].ToString() + " AND m_room.room_cat_id = m_rent.room_category AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "') ";
                    DataTable dt_rent = objcls.DtTbl(n_rent);
                    if (dt_rent.Rows.Count > 0)
                    { //dt_rent.Rows[0][0].ToString()
                        txtroomrent.Text = "0";
                        txtsecuritydeposit.Text = dt_rent.Rows[0][1].ToString();
                        double tot = Convert.ToDouble(0) + Convert.ToDouble(dt_rent.Rows[0][1].ToString()) + Convert.ToDouble(dtreserve.Rows[0]["other_charge"].ToString());
                        txttotalamount.Text = tot.ToString();
                        txtnetpayment0.Text = Convert.ToString(tot - Convert.ToDouble(dtreserve.Rows[0]["advance"].ToString()));
                    }
                }
    ...
    private void resno()
    {
        ...
        if (txtReserveNo.Text != "")
        {
            ...
            string reservecheck = "SELECT DISTINCT t_roomreservation.reserve_mode,t_roomreservation_generaltdbtemp.swaminame,t_roomreservation_generaltdbtemp.place, t_roomreservation_generaltdbtemp.std,t_roomreservation_generaltdbtemp.phone,t_roomreservation_generaltdbtemp.district_id, t_roomreservation_generaltdbtemp.state_id, m_room.build_id,t_roomreservation.room_id, IF(t_roomreservation_generaltdbtemp.reservedate > NOW(),DATE_FORMAT(NOW(),'%m/%d/%Y %r'), DATE_FORMAT(t_roomreservation_generaltdbtemp.reservedate,'%m/%d/%Y %r')) AS 'chkin', DATE_FORMAT(t_roomreservation_generaltdbtemp.expvacdate,'%m/%d/%Y %r') AS 'chkout', t_roomreservation_generaltdbtemp.total_days,t_roomreservation_generaltdbtemp.inmates_mobile_no, t_roomreservation_generaltdbtemp.inmates_email,t_roomreservation_generaltdbtemp.proof_id, t_roomreservation_generaltdbtemp.proof_no,t_roomreservation_generaltdbtemp.room_rent, t_roomreservation_generaltdbtemp.advance,t_roomreservation_generaltdbtemp.security_deposit, t_roomreservation_generaltdbtemp.res_charge,t_roomreservation_generaltdbtemp.other_charge, t_roomreservation_generaltdbtemp.total_charge,t_roomreservation_generaltdbtemp.balance_amount, t_roomreservation_generaltdbtemp.season_sub_id,t_roomreservation_generaltdbtemp.inmates_no, t_roomreservation_generaltdbtemp.reserve_hours,t_roomreservation_generaltdbtemp.adv_recieptno, IF(t_roomreservation_generaltdbtemp.reservedate > NOW(),DATE_FORMAT(NOW(),'%Y/%m/%d %r'), DATE_FORMAT(t_roomreservation_generaltdbtemp.reservedate,'%Y/%m/%d %r')) AS 'chkin_dup',  DATE_FORMAT(t_roomreservation_generaltdbtemp.expvacdate,'%Y/%m/%d %r') AS 'chkout_dup',t_roomreservation.pass_id,t_roomreservation.reserve_id ,   DATE_FORMAT(t_roomreservation_generaltdbtemp.reservedate,'%Y/%m/%d %r') AS 'org_chkin' FROM t_roomreservation,t_roomreservation_generaltdbtemp,m_room  WHERE t_roomreservation_generaltdbtemp.reserve_no=t_roomreservation.reserve_no  AND m_room.room_id = t_roomreservation.room_id  AND t_roomreservation_generaltdbtemp.reserve_no='" + txtReserveNo.Text + "'  AND t_roomreservation_generaltdbtemp.status_reserve = '0'   AND t_roomreservation.status_reserve = '0'  AND t_roomreservation_generaltdbtemp.reserve_mode= IF(t_roomreservation_generaltdbtemp.status_type= 0 , 'Donor','Donor Free') AND 0 = (SELECT t_donorpass.passtype FROM t_donorpass WHERE t_donorpass.pass_id =t_roomreservation.pass_id)";
            if (Session["clubbing"].ToString() == "ok")
            {
                if (cmballoctype.SelectedValue == "Donor Paid Allocation")
                {
                    reservecheck = "SELECT DISTINCT t_roomreservation.reserve_mode,t_roomreservation_generaltdbtemp.swaminame,t_roomreservation_generaltdbtemp.place, t_roomreservation_generaltdbtemp.std,t_roomreservation_generaltdbtemp.phone,t_roomreservation_generaltdbtemp.district_id, t_roomreservation_generaltdbtemp.state_id, m_room.build_id,t_roomreservation.room_id, IF(t_roomreservation_generaltdbtemp.reservedate > NOW(),DATE_FORMAT(NOW(),'%m/%d/%Y %r'), DATE_FORMAT(t_roomreservation_generaltdbtemp.reservedate,'%m/%d/%Y %r')) AS 'chkin', DATE_FORMAT(t_roomreservation_generaltdbtemp.expvacdate,'%m/%d/%Y %r') AS 'chkout', t_roomreservation_generaltdbtemp.total_days,t_roomreservation_generaltdbtemp.inmates_mobile_no, t_roomreservation_generaltdbtemp.inmates_email,t_roomreservation_generaltdbtemp.proof_id, t_roomreservation_generaltdbtemp.proof_no,t_roomreservation_generaltdbtemp.room_rent, t_roomreservation_generaltdbtemp.advance,t_roomreservation_generaltdbtemp.security_deposit, t_roomreservation_generaltdbtemp.res_charge,t_roomreservation_generaltdbtemp.other_charge, t_roomreservation_generaltdbtemp.total_charge,t_roomreservation_generaltdbtemp.balance_amount, t_roomreservation_generaltdbtemp.season_sub_id,t_roomreservation_generaltdbtemp.inmates_no, t_roomreservation_generaltdbtemp.reserve_hours,t_roomreservation_generaltdbtemp.adv_recieptno, IF(t_roomreservation_generaltdbtemp.reservedate > NOW(),DATE_FORMAT(NOW(),'%Y/%m/%d %r'), DATE_FORMAT(t_roomreservation_generaltdbtemp.reservedate,'%Y/%m/%d %r')) AS 'chkin_dup',  DATE_FORMAT(t_roomreservation_generaltdbtemp.expvacdate,'%Y/%m/%d %r') AS 'chkout_dup',t_roomreservation.pass_id,t_roomreservation.reserve_id,   DATE_FORMAT(t_roomreservation_generaltdbtemp.reservedate,'%Y/%m/%d %r') AS 'org_chkin'  FROM t_roomreservation,t_roomreservation_generaltdbtemp,m_room  WHERE t_roomreservation_generaltdbtemp.reserve_no=t_roomreservation.reserve_no  AND m_room.room_id = t_roomreservation.room_id  AND t_roomreservation_generaltdbtemp.reserve_no='" + txtReserveNo.Text + "'  AND t_roomreservation_generaltdbtemp.status_reserve = '0'  AND t_roomreservation.status_reserve = '0'  AND t_roomreservation_generaltdbtemp.reserve_mode= IF(t_roomreservation_generaltdbtemp.status_type= 0 , 'Donor','Donor Paid') AND 1 = (SELECT t_donorpass.passtype FROM t_donorpass WHERE t_donorpass.pass_id =t_roomreservation.pass_id)";
            ...
            if (dtreserve.Rows.Count > 0)
            {
                ...
                if (Session["clubbing"].ToString() == "ok")
                {
                    ...
                    tcheck = @"SELECT TIMEDIFF( STR_TO_DATE('" + dtreserve.Rows[0]["chkout_dup"].ToString() + "','%Y/%m/%d %l:%i:%s %p'),STR_TO_DATE('" + dtreserve.Rows[0]["org_chkin"].ToString() + "','%Y/%m/%d %l:%i:%s %p'))";
                     dt_tcheck = objcls.DtTbl(tcheck);
                  //  int hour = 0;
                    if (dt_tcheck.Rows.Count > 0)
                    {
                        TimeSpan actperiod = TimeSpan.Parse(dt_tcheck.Rows[0][0].ToString());
                    ...
                    string fromtym = txtcheckindate.Text + " " + txtcheckintime.Text;
                    ...
                    if (cmballoctype.SelectedValue != "Donor Free Allocation")
                    {
                        ...
                        {
                            hour = 12;
                            txtnoofdays.Text = Convert.ToString(hour);
                        }

                    string add = @"SELECT CAST(DATE_FORMAT(ADDTIME(STR_TO_DATE('" + fromtym + "','%d/%m/%Y %l:%i %p'),'" + hour + ":00:00'),'%Y-%m-%d %r') AS CHAR(30))";
                    DataTable dt_add = objcls.DtTbl(add);
                    DateTime fin_in = Convert.ToDateTime(dt_add.Rows[0][0].ToString());
                    ...
                    else
                    {
                        ...
                        string st = @"SELECT  CASE  WHEN STR_TO_DATE('" + fromtym + "','%d/%m/%Y %l:%i %p') < STR_TO_DATE(CONCAT('" + txtcheckindate.Text + "',' 3:00 pm'),'%d/%m/%Y %l:%i %p') THEN CONCAT(DATE_FORMAT('" + objcls.yearmonthdate(txtcheckindate.Text) + "','%d/%m/%Y'),' 3:00 pm') ELSE CONCAT(DATE_FORMAT(ADDDATE('" + objcls.yearmonthdate(txtcheckindate.Text) + "',INTERVAL 1 DAY),'%d/%m/%Y'),' 3:00 pm') END AS 'chk' ";
                        DataTable dt_st = objcls.DtTbl(st);
                        if (dt_st.Rows.Count > 0)
                        {
                            string[] splitdate = (dt_st.Rows[0][0].ToString()).Split(' ');
                            ...
                            txtcheckout.Text = splitdate[0].ToString();
                            txtcheckouttime.Text = splitdate[1].ToString() + " " + splitdate[2].ToString();
                            string totym = txtcheckout.Text + " " + txtcheckouttime.Text;


                            string st1 = @"SELECT TIMEDIFF(STR_TO_DATE('" + totym + "','%d/%m/%Y %l:%i %p'),STR_TO_DATE('" + fromtym + "','%d/%m/%Y %l:%i %p')) ";
                            DataTable dt_st1 = objcls.DtTbl(st1);
                            if (dt_st1.Rows.Count > 0)
                            {
...
FROM m_rent,
  m_room
WHERE (" + hour + " > m_rent.start_duration) AND (" + hour + " <= m_rent.end_duration) AND m_room.room_id = " + dtreserve.Rows[0]["room_id"].ToString() + " AND m_room.build_id = " + dtreserve.Rows[0]["build_id"].ToString() + " AND m_room.room_cat_id = m_rent.room_category AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "') ";
                            DataTable dt_rent = objcls.DtTbl(n_rent);
                            if (dt_rent.Rows.Count > 0)
                            { //dt_rent.Rows[0][0].ToString()
                                if (cmballoctype.SelectedValue != "Donor Free Allocation")
                        ...
                        OdbcCommand cmdRomxx = new OdbcCommand();
                        cmdRomxx.Parameters.AddWithValue("tblname", "m_room");
                        cmdRomxx.Parameters.AddWithValue("attribute", "distinct roomno,room_id");
                        //cmdRom.Parameters.AddWithValue("conditionv", "build_id =" + int.Parse(cmbBuild.SelectedValue.ToString()) + " and  rowstatus<>" + 2 + " and roomstatus=" + 1 + " order by roomno asc");
                        cmdRomxx.Parameters.AddWithValue("conditionv", "rowstatus <> 2 order by roomno asc");
                        OdbcDataReader drrxx = objcls.SpGetReader("CALL selectcond(?,?,?)", cmdRomxx);
                        DataTable dtt36xx = new DataTable();
                        dtt36xx = objcls.GetTable(drrxx);
                        DataRow row = dtt36xx.NewRow();
                        row["room_id"] = "-1";
                        row["roomno"] = "--Select--";
                        dtt36xx.Rows.InsertAt(row, 0);
                        dtt36xx.AcceptChanges();
                        cmbRooms.DataSource = dtt36xx;
                        cmbRooms.DataBind();
                        cmbRooms.SelectedValue = dtreserve.Rows[0]["room_id"].ToString();
                        Session["reserv"] = "ok";
                        Session["roomrent"] = txtroomrent.Text;
                    ...
                    string maxix = @"SELECT max_allocdays FROM t_policy_allocation WHERE reqtype='"+cmballoctype.SelectedItem.ToString()+"' AND CURDATE() BETWEEN fromdate AND todate";
                    DataTable dt_maxix = objcls.DtTbl(maxix);
                   // int max_check = 0;
                    if (dt_maxi.Rows.Count > 0)
                    {
                        max_check = Convert.ToInt16(dt_maxix.Rows[0][0].ToString());
                    }
...
WHERE (" + noofhours + " > m_rent.start_duration) AND (" + noofhours + " <= m_rent.end_duration) AND m_room.room_id = " + dtreserve.Rows[0]["room_id"].ToString() + " AND m_room.build_id = " + dtreserve.Rows[0]["build_id"].ToString() + " AND m_room.room_cat_id = m_rent.room_category AND m_rent.reservation_type = (SELECT id FROM p_type_of_user WHERE TYPE = '" + cmballoctype.SelectedValue + "') ";
                ...
                DataTable dtt36 = new DataTable();
                dtt36 = objcls.GetTable(drr);
                DataRow rowxx = dtt36.NewRow();
                rowxx["room_id"] = "-1";
                rowxx["roomno"] = "--Select--";
                dtt36.Rows.InsertAt(rowxx, 0);
                dtt36.AcceptChanges();
                cmbRooms.DataSource = dtt36;
                cmbRooms.DataBind();
                cmbRooms.SelectedValue = dtreserve.Rows[0]["room_id"].ToString();
    ...
    protected void btnclubadd_Click(object sender, EventArgs e)
    {

        if (Session["clubbing"] == "ok")
        {

            if (cmballoctype.SelectedValue == "--Select--")
            {
                okmessage("Tsunami ARMS - Warning", "This policy can't be clubbed");
                return;
            }



            string passno = "0",other = "0",inmatecharge="0",inmatedeposit="0",granttot="0",reserve="0",advance="0",balpayable="0",building="0",room="0";
            if (txtdonorpass.Text != "")
            {
                passno = txtdonorpass.Text;
            }
            if (txtothercharge.Text != "")
            {
                other = txtothercharge.Text;
            }
            if (txtinmatecharge.Text != "")
            {
                inmatecharge = txtinmatecharge.Text;
            }
            if (txtinmatedeposit.Text != "")
            {
                inmatedeposit = txtinmatedeposit.Text;
            }
            if(txtgranttotal.Text!="")
            {
                granttot=txtgranttotal.Text;
            }
            if (txtReserveNo.Text != "")
            {
                reserve = txtReserveNo.Text;
            }
            if (txtadvance.Text != "")
            {
                advance = txtadvance.Text;
            }
            if (txtnetpayment0.Text != "")
            {
                balpayable = txtnetpayment0.Text;
            }
            if (cmbBuild.SelectedItem.Text != "")
            {
                building = cmbBuild.SelectedItem.Text;
            }
            if (cmbRooms.SelectedItem.Text != "")
            {
                room = cmbRooms.SelectedItem.Text;
            }


            DataTable dt_club = Session["club"] as DataTable;
            int cnt = dt_club.Rows.Count;
            DataRow dr = dt_club.NewRow();
            dr["Type"] = cmballoctype.SelectedValue;
            dr["Checkin"] = txtcheckindate.Text + " " + txtcheckintime.Text;
            dr["Checkout"] = txtcheckout.Text + " " + txtcheckouttime.Text;
            dr["No of hours"] = txtnoofdays.Text;
            dr["Passno"] = passno;
            dr["Rent"] = txtroomrent.Text;
            dr["Deposit"] = txtsecuritydeposit.Text;
            dr["Othercharge"] = other;
            dr["Inmatecharge"] = inmatecharge;
            dr["Inmatedeposit"] = inmatedeposit;
            dr["Reserve No"] = reserve;
            dr["Building"] = building;
            dr["Room"] = room;
            dt_club.Rows.InsertAt(dr, cnt);


            gvclub.Visible = true;
            gvclub.DataSource = dt_club;
            gvclub.DataBind();

            ViewState["txtroomrent"] = (Convert.ToInt32(ViewState["txtroomrent"].ToString()) + Convert.ToInt32(txtroomrent.Text)).ToString();
            ViewState["txtsecuritydeposit"] = (Convert.ToInt32(ViewState["txtsecuritydeposit"].ToString()) + Convert.ToInt32(txtsecuritydeposit.Text)).ToString();
            ViewState["hours"] = (Convert.ToInt32(ViewState["hours"].ToString()) + Convert.ToInt32(txtnoofdays.Text)).ToString();
            ViewState["txtinmatecharge"] = (Convert.ToInt32(ViewState["txtinmatecharge"].ToString()) + Convert.ToInt32(txtinmatecharge.Text)).ToString();
            ViewState["txtgranttotal"] = (Convert.ToInt32(ViewState["txtgranttotal"].ToString()) + Convert.ToInt32(txtgranttotal.Text)).ToString();
            ViewState["txtinmatedeposit"] = (Convert.ToInt32(ViewState["txtinmatedeposit"].ToString()) + Convert.ToInt32(txtinmatedeposit.Text)).ToString();
            ViewState["txtadvance"] = Convert.ToInt32(txtadvance.Text) + Convert.ToInt32(ViewState["txtadvance"].ToString());
            ViewState["txtnetpayment0"] = Convert.ToInt32(txtnetpayment0.Text) + Convert.ToInt32(ViewState["txtnetpayment0"].ToString());
            ViewState["txtothercharge"] = Convert.ToInt32(txtothercharge.Text) + Convert.ToInt32(ViewState["txtothercharge"].ToString()); 


            Session["club"] = dt_club;
            
            ViewState["startclub"] = "ok";//to ensure grid is loaded

            txtcheckindate.Text = txtcheckout.Text;
            txtcheckintime.Text = txtcheckouttime.Text;
            txtnoofdays.Text =  ViewState["hours"].ToString();
            txtdonorpass.Text = "";
            txtroomrent.Text = ViewState["txtroomrent"].ToString();
            txtsecuritydeposit.Text = ViewState["txtsecuritydeposit"].ToString();

            txtcheckout.Text = "";
            txtcheckouttime.Text = "";


            txttotalamount.Text = (Convert.ToInt32(ViewState["txtroomrent"].ToString()) + Convert.ToInt32(ViewState["txtsecuritydeposit"].ToString()) + Convert.ToInt32(ViewState["txtothercharge"].ToString())).ToString();
            txtinmatecharge.Text = ViewState["txtinmatecharge"].ToString();
            txtinmatedeposit.Text = ViewState["txtinmatedeposit"].ToString();
            txtgranttotal.Text = ViewState["txtgranttotal"].ToString();
            txtothercharge.Text = ViewState["txtothercharge"].ToString();
            txtinmatedeposit.Text = ViewState["txtinmatedeposit"].ToString();
            txtadvance.Text =  ViewState["txtadvance"].ToString();
            txtnetpayment0.Text = ViewState["txtnetpayment0"].ToString();
    ...
    protected void rbclubbing_CheckedChanged(object sender, EventArgs e)
    {
        if (chkclubbing.Checked == true)
        {
            cmballoctype.Enabled = true;
            Session["clubbing"] = "ok";
            btnclubadd.Visible = true;

            try
            {
                OdbcCommand cmdDCclub = new OdbcCommand();
                cmdDCclub.Parameters.AddWithValue("tblname", "p_clubbing INNER JOIN p_type_of_user ON p_type_of_user.id = p_clubbing.reserve_types  ");
                cmdDCclub.Parameters.AddWithValue("attribute", "DISTINCT TYPE ");
                cmdDCclub.Parameters.AddWithValue("conditionv", "CURDATE() BETWEEN from_date AND to_date AND  clubbing_status= '1'");
                DataTable dtDCclub = new DataTable();
                dtDCclub = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdDCclub);
            ...
            {
                okmessage("Tsunami ARMS - Warning", "clubbing policies not set.");
                this.ScriptManager1.SetFocus(btnOk);
                string prevpage = Request.UrlReferrer.ToString();
                Response.Redirect(prevpage);
            }
    ...
    protected void cmballoctype_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (cmballoctype.SelectedValue == "General Allocation")
        {
            //cmbRooms.Enabled = true;
            //cmbBuild.Enabled = true;
            txtdonorpass.Text = "";

            if (ViewState["gchk"].ToString() == "ok")
            {
                try
                {
                    string rrmm = Session["room"].ToString();
                        ...
                        else
                        {
                            ...
                            if (Session["Clubbing"] == "ok" && (cmballoctype.SelectedValue != "Donor Free Allocation" || cmballoctype.SelectedValue != "Donor Paid Allocation"))
                            {
                                ...
                                if (ViewState["startclub"].ToString() == "ok")
                                {
                                    ...
                                    for (int i = 0; i < gvclub.Rows.Count; i++)
                                    {
                                        if (gvclub.Rows[i].Cells[0].Text == "General Allocation")
                                        {
                                            tyme = Convert.ToInt32(gvclub.Rows[i].Cells[3].Text) + tyme;                                                                                       
                                        }
                                    }
            ...
            {
                ViewState["auction"] = "no";
                okmessage("Tsunami ARMS - Complaint", "Select donor paid/free first");
                this.ScriptManager1.SetFocus(btnOk);
            }
