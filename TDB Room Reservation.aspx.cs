...
public partial class TDB_Room_Reservation : System.Web.UI.Page
{
    #region Declarations
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack == true)
        {
            ...
            Title = "Tsunami ARMS - TDB Reservation";
        ...
        n = int.Parse(Session["userid"].ToString());
        this.ScriptManager1.SetFocus(txtOfficer);  
    }
    ...
    private void load()
    {
        ...
        cmdSxcvb.Parameters.AddWithValue("conditionv", " tv.dayend>=(SELECT fromdate FROM  t_policy_allocation WHERE reqtype = 'TDB Allocation' AND CURDATE() BETWEEN fromdate AND todate  ORDER BY alloc_policy_id DESC LIMIT 1)  AND tv.dayend<=(SELECT todate FROM  t_policy_allocation WHERE reqtype = 'TDB Allocation'  AND CURDATE() BETWEEN fromdate AND todate ORDER BY alloc_policy_id DESC LIMIT 1)  AND   msb.build_id=mr.build_id AND mr.room_id=ta.room_id AND ta.alloc_id=tv.alloc_id  AND inmate_abscond=1 AND tv.counter_id =ta.counter_id AND tv.counter_id = '" + Session["counter"].ToString() + "'  ORDER BY adv_recieptno ");
        ...
        if (ViewState["action"].ToString() == "save")
        {
            ...
            if (custtype == "TDB")
            {
                ...
                try
                {
                    ...
                    string generalreserve = "INSERT INTO t_roomreservation_generaltdbtemp(reserve_id,reserve_type,reserve_mode,multi_slno,swaminame,reservedate,expvacdate,total_days,status_reserve,createdby,createdon,updatedby,updateddate,place,room_category_id,status_type,inmates_mobile_no,inmates_email,proof_id,proof_no,room_rent,advance,security_deposit,res_charge,other_charge,total_charge,inmates_no,reserve_hours,adv_recieptno,is_plainprint,balance_amount,counter_id,season_sub_id,STD,phone,district_id,state_id,reserve_no,allot_status) VALUES ( " + temp + ",'Single','" + custtype + "'," + typeno + ",'" + txtSwaminame.Text.ToString() + "','" + fromdate.ToString() + "','" + todate.ToString() + "'," + txtnoofhours.Text + ",0," + n + ",'" + date.ToString() + "'," + n + ",'" + date.ToString() + "','" + txtPlace.Text + "','" + cmbroomcategory.SelectedValue + "','1'," + txtMobileNo.Text + ",'" + txtEmail.Text + "'," + cmbProofType.SelectedValue + ",'" + txtProofNo.Text + "'," + txtroomrent.Text + "," + txtadvance.Text + "," + txtsecuritydeposit.Text + "," + txtrservtnchrge.Text + "," + txtothercharge.Text + "," + txttotalamount.Text + "," + txtnoofinmates.Text + "," + txtnoofhours.Text + "," + txtreceiptno1.Text + ",'" + pprintrec + "'," + txtnetpayable.Text + "," + counter + "," + season_sub_id + "," + txtStd.Text + "," + txtPhn.Text + "," + cmbDistrict.SelectedValue + "," + cmbState.SelectedValue + ",'" + tempnew + "',1)";                    
                    OdbcCommand reserve = new OdbcCommand(generalreserve, con);
                    reserve.Transaction = odbTrans;
                    reserveconfirm = reserve.ExecuteNonQuery();

                    string reserveroom = "INSERT INTO t_roomreservation(reserve_id,reserve_type,reserve_mode,swaminame,reservedate,expvacdate,total_days,status_reserve,passmode,createdby,cretaedon,updatedby,updateddate,place,room_id,reserve_no) VALUES ( " + pk + ",'Single','TDB','" + txtSwaminame.Text.ToString() + "','" + fromdate + "','" + todate + "'," + txtnoofhours.Text + ",0,0," + n + ",'" + date + "'," + n + ",'" + date + "','" + txtPlace.Text + "'," + cmbRooms.SelectedValue + ",'" + tempnew + "')";
                    ...
                    string a = @"UPDATE p_roomstatus SET rooms_allowed=rooms_allowed-1,room_status=room_status+1 WHERE room_category_id='" + cmbroomcategory.SelectedValue + "' AND season_sub_id=" + season_sub_id + " AND type_id=3 AND date_in='" + fromdate.ToString() + "'";
                    OdbcCommand updateroom = new OdbcCommand(a, con);
                    updateroom.Transaction = odbTrans;
                    updateroom.ExecuteNonQuery();
                    //objcls.exeNonQuery(a);
                # endregion                    
    ...
    protected void txtchkin_TextChanged(object sender, EventArgs e)
    {
        if (txtFrmdate.Text != "")
        {
            ...
            string category = @"SELECT room_category_id,rooms_allowed,season_sub_id FROM p_roomstatus WHERE date_in='" + objcls.yearmonthdate(txtFrmdate.Text) + "' AND type_id=3";
        ...
        if ((txtFrmdate.Text != "") && (txtchkin.Text != "") && (txtTodate.Text != "") && (txtchkout.Text != ""))
        {
            ...
            {
                string dur = @"select day_res_max from t_policy_reservation INNER JOIN t_policy_reserv_seasons ON t_policy_reserv_seasons.res_policy_id=t_policy_reservation.res_policy_id  WHERE season_sub_id=" + dtss.Rows[0][1].ToString() + " AND res_type='Tdb' AND CURDATE() BETWEEN t_policy_reservation.res_from AND t_policy_reservation.res_to";
    ...
    protected void btnreservelist_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            cmd31.Parameters.AddWithValue("attribute", "DISTINCT t.reserve_no,t.place,t.reservedate 'Reserve from',t.expvacdate 'Reserve To',case reserve_mode when 'TDB' then 'TDB' END as 'Customer Type',t.swaminame,status_reserve,CASE  WHEN (SELECT DISTINCT reserve_id FROM t_roomallocation  WHERE t_roomallocation.reserve_id = t.reserve_no) != '' THEN 'allocated' ELSE 'not allocated' END AS 'status' ");
              cmd31.Parameters.AddWithValue("conditionv", "DATE_FORMAT(reservedate,'%Y/%m/%d') >=  '" + str1.ToString() + "' and reserve_mode='TDB' and DATE_FORMAT(reservedate,'%Y/%m/%d') <= '" + str2.ToString() + "' ORDER BY t.reserve_id  ASC");
    ...
    public void generalpdf()
    {
        ...
        PdfPCell cell012 = new PdfPCell(new Phrase("TDB Reservation Form", font5));
        cell012.Border = 0;
        ...
        PdfPCell cell8 = new PdfPCell(new Phrase(":TDB", ti8normal));
        cell8.Border = 0;
        ...
        PdfPCell cell10 = new PdfPCell(new Phrase(": " + cmbroomcategory.Text, ti8bold));
        cell10.Border = 0;
...
2.	The reservation chart for " + reservedate + "will be published on" + publishdate + " . Check and confirm your room " + "\n" + "3.	Checkin should be done at the time specified in the form. In case of any change required, please convey it by Phone to 04735 220928  " + "\n" + "4.	Produce payment confirmation note at accommodation office and deposit security deposit to avail room. " + "\n" + "5.	Rent and reservation charge mentioned here is based on current rate and Devaswom Board reserve the right to change these charges without notice. Inmates will have to pay actual rent and reservation charge at the time of Check-In. " + "\n" + "6.	Devaswom board reserve the right to change the room, check-in time and pilgrim has to cooperate with the board " + "\n" + "7.	In the event of any urgency, the donor pilgrim will have to vacate the room or surrender reservation.    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ", ti8normal);
    ...
    private void roomcategory()
    {
        ...
        {
            Session["season_sub_idnew"] = dt1.Rows[0]["season_sub_id"].ToString();
            string roomcat = @"SELECT DISTINCT m_sub_room_category.room_cat_id,m_sub_room_category.room_cat_name FROM p_roomstatus,m_sub_room_category WHERE p_roomstatus.room_category_id=m_sub_room_category.room_cat_id AND p_roomstatus.type_id=3 and p_roomstatus.date_in='" + objcls.yearmonthdate(txtFrmdate.Text) + "'";
            DataTable dtcat = objcls.DtTbl(roomcat);
            DataRow row11b = dtcat.NewRow();
            row11b["room_cat_id"] = "-1";
            row11b["room_cat_name"] = "--Select--";
    ...
    protected void cmbroomcategory_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (dtreservepolicy.Rows.Count > 0)
        {
            ...
            try
            {
                string roomcat = @"SELECT m_sub_room_category.room_cat_name,p_roomstatus.rooms_allowed,p_roomstatus.season_sub_id FROM p_roomstatus,m_sub_room_category WHERE m_sub_room_category.room_cat_id=p_roomstatus.room_category_id AND p_roomstatus.type_id=3 AND p_roomstatus.date_in='" + objcls.yearmonthdate(txtFrmdate.Text) + "'";
                DataTable dtcat = objcls.DtTbl(roomcat);
    ...
    {
        string rentdetails = "SELECT rent,security_deposit,reserve_charge FROM m_rent WHERE reservation_type=" + 3 + " AND room_category=" + cmbroomcategory.SelectedValue + " and '" + txtnoofhours.Text + "' BETWEEN start_duration AND end_duration";
    ...
    protected void txtadvance_TextChanged(object sender, EventArgs e)
    {
        ...
        if ((txtadvance.Text != "") && (txttotalamount.Text != ""))
        {
            int total = int.Parse(txttotalamount.Text);
            int advancecal = int.Parse(txtadvance.Text);
            txtnetpayable.Text = (total - advancecal).ToString();
        }
    }
    ...
    {
//        if (cmbProofType.SelectedValue != "--Select--")
//        {
