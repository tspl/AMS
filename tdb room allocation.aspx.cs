...
public partial class tdb_room_allocation : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                try
                {
                    ...
                    DateTime date1 = DateTime.ParseExact(dt_nw.Rows[0][0].ToString(), "dd/MM/yyyy", System.Globalization.CultureInfo.InvariantCulture);
    ...
    public void roomrentcalculate()
    {
        try
        {
            ...
            if (defhour > 0)
            {
                ...
                {
                    //string renttt = @"SELECT m_rent.rent,m_rent.security_deposit FROM m_rent ,m_room WHERE " + defhour + " > m_rent.start_duration)  AND (" + defhour + "'<= m_rent.end_duration ) AND m_room.room_id = " + cmbRooms.SelectedValue + " AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  m_room.room_cat_id = m_rent.room_category AND reservation_type=3 ";
                    string renttt = @"SELECT m_rent.rent,m_rent.security_deposit 
FROM m_rent ,m_room 
WHERE (" + defhour + " > m_rent.start_duration)  AND (" + defhour + " <= m_rent.end_duration ) AND m_room.room_id = " + cmbRooms.SelectedValue + " AND  m_room.build_id = " + cmbBuild.SelectedValue + "  AND  m_room.room_cat_id = m_rent.room_category AND reservation_type=3";
                    ...
                    cmdRR.Parameters.AddWithValue("conditionv", " ('" + defhour + "' > m_rent.start_duration)  AND ('" + defhour + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  m_room.room_cat_id = m_rent.room_category AND m_rent.reservation_type = '3' ");
                    DataTable dtRR = new DataTable();
                    //dtRR = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdRR);
                    dtRR = objcls.DtTbl(renttt);
            ...
            if (Session["altcalc"].ToString() == "ok")
            {
                ...
                cmdRR.Parameters.AddWithValue("tblname", " m_rent ,m_room");
                cmdRR.Parameters.AddWithValue("attribute", " m_rent.rent,m_rent.security_deposit");
                cmdRR.Parameters.AddWithValue("conditionv", " ('" + tottime + "' > m_rent.start_duration)  AND ('" + tottime + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  m_room.room_cat_id = m_rent.room_category AND m_rent.reservation_type = '3'");
                DataTable dtRR = new DataTable();
                dtRR = objcls.SpDtTbl("CALL selectcond(?,?,?)", cmdRR);
                txtroomrent.Text = dtRR.Rows[0]["rent"].ToString();
                txtsecuritydeposit.Text = dtRR.Rows[0]["security_deposit"].ToString();
    ...
    public void daterent()
    {
        try
        {
            if (txtcheckout.Text != "")
            {
                ...
                //String SS = "SELECT TIMEDIFF(STR_TO_DATE('" + odate + "','%Y-%m-%d %l:%i:%s %p'), NOW())";
    ...
    public void rentcalculation()
    {
        try
        {
            ...
            if (txtcheckout.Text != "" && lblhead.Text == "GENERAL ALLOCATION")
            {
                ...
                cmdR.Parameters.AddWithValue("conditionv", " ('" + n + "' > m_rent.start_duration)  AND ('" + n + "' <= m_rent.end_duration ) AND m_room.room_id = '" + cmbRooms.SelectedValue + "' AND  m_room.build_id = '" + cmbBuild.SelectedValue + "'  AND  room_cat_id = m_rent.room_category AND m_rent.reservation_type = '3' ");
    ...
    public void generalallocationbuilding()
    {
        try
        {
            ...
            //    //string strSql4 = "SELECT distinct build.buildingname,build.build_id FROM m_sub_building as build,m_room as room WHERE room.build_id=build.build_id and room.roomstatus=" + 1 + " and room.rowstatus<>" + 2 + " order by build.buildingname asc";
                ...
                string rm = @"select distinct build.buildingname,build.build_id from m_sub_building as build,m_room as room where room.build_id=build.build_id and room.roomstatus=" + 1 + " and room.rowstatus<>" + 2 + " order by build.buildingname asc";
    ...
    public void AllocationSave()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                string hhh = @"Select max(alloc_id) from t_roomallocation";
                ...
                DataTable dtt90 = objcls.DtTbl(hhh);
              //dtt90
              //  dacnt90.Fill(dtt90);
            ...
            OdbcCommand cmdssxc = new OdbcCommand("SELECT NOW()", con);
            cmdssxc.Transaction = odbTrans;
            DateTime update = Convert.ToDateTime(cmdssxc.ExecuteScalar());
           // DateTime update = DateTime.Now;
            ...
            DateTime curYear = Convert.ToDateTime(cmdss.ExecuteScalar());
            ...
            string reservepolicy = "SELECT is_rent,is_deposit FROM t_policy_reservation WHERE res_type='Tdb' AND '" + curYear.ToString("yyyy-MM-dd") + "'  BETWEEN res_from AND res_to";
            ...
            if (Session["reserv"].ToString() == "ok")
            {
                ...
                string uosaas = "update t_roomreservation_generaltdbtemp set status_reserve=" + 2 + " where reserve_no = '" + txtReserveNo.Text + "' AND reserve_mode='TDB'";
            }
            ...
            bal = bal + depo;
            string savdep = "'" + int.Parse(Session["counter"].ToString()) + "','" + int.Parse(Session["counter"].ToString()) + "','" + int.Parse(Session["userid"].ToString()) + "','" + curseason2 + "','" + int.Parse(Session["malYear"].ToString()) + "','" + date + "',1,'" + id + "','" + depo + "','" + bal + "'";
    ...
    protected void txtcheckout_TextChanged(object sender, EventArgs e)
    {
                ...
                try
                {
                    ...
                    if (strin == strout)
                    {
                        ...
                        if (outdxx < indxx)
                        {
                            ...
                            txtcheckouttime.Text = ""; // "00:00:00 " + checkinSplit[1];
                            this.ScriptManager1.SetFocus(btnOk);
                            return;
                        }
        ...
        {
            //okmessage("Tsunami ARMS - Warning", "Enter date and time");
            //this.ScriptManager1.SetFocus(btnOk);
            //return;
        }
    }
    ...
    protected void btnallocate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            string add = @"Select adv_recieptno from t_roomallocation where adv_recieptno=" + int.Parse(txtreceiptno1.Text) + " and is_plainprint='" + RecOld + "' AND alloc_type='TDB Allocation'";
        ...
        try
        {
            ...
            if (dtS.Rows.Count > 0)
            {
                ...
                //string ab = @"Select alloc_policy_id from t_policy_allocation_seasons where season_sub_id=" + curseason + " and rowstatus <> " + 2 + " ";
                OdbcCommand cmdAPS = new OdbcCommand();
                cmdAPS.Parameters.AddWithValue("tblname", "t_policy_allocation_seasons");
                cmdAPS.Parameters.AddWithValue("attribute", "alloc_policy_id");
                cmdAPS.Parameters.AddWithValue("conditionv", "season_sub_id=" + curseason + " and rowstatus <> " + 2 + "");
                ...
                if (dtAPS.Rows.Count > 0)
                {
                    ...
                    for (int ii = 0; ii < dtAPS.Rows.Count; ii++)
                    {
                        ...
                        int sid = int.Parse(dtAPS.Rows[ii]["alloc_policy_id"].ToString());
                        string gggg = Session["allotype"].ToString();
                        string abc = @"Select max_allocdays from t_policy_allocation where alloc_policy_id=" + sid + "  and reqtype='" + gggg + "' and (curdate() between fromdate and todate) or (curdate()>=fromdate and todate='0000-00-00') and rowstatus<>" + 2 + " AND reqtype='TDB Allocation' ";
    ...
    public void reservation()
    {
        try
        {
            ...
            try
            {
                ...
                string abcd = @"Select max(reserve_id) from t_roomreservation WHERE reserve_type='TDB Allocation'";
        ...
        try
        {
            ...
            if ((btncancel.Enabled == false) || (btncancel.Text == "Cancel Alloc"))
            {
                try
                {
                    ...
                    string abcde = @"select swaminame,place,state_id,district_id,phone,idproof,idproofno,recieptno,roomrent,deposit,advance,othercharge,reason,totalcharge from t_roomallocation where alloc_id=" + q + " AND alloc_type='General Allocation'";
    ...
    protected void btnsave_Click2(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string bcd = @"Select adv_recieptno from t_roomallocation adv_recieptno=" + int.Parse(txtreceiptno1.Text) + " and is_plainprint='" + RecOld + "' and alloc_type='General Allocation'";
        ...
        try
        {
            ...
            string ddd = @"select adv_recieptno from t_roomallocation from counter_id=" + int.Parse(Session["counter"].ToString()) + " and is_plainprint='" + RecOld + "' and roomstatus<>'null' order by alloc_id desc limit 0,1 and alloc_type='General Allocation'";
    ...
    protected void cmbaltbulilding_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            //cmdRR.Parameters.AddWithValue("tblname", " m_rent ,m_room");
            //cmdRR.Parameters.AddWithValue("attribute", " m_rent.rent,m_rent.security_deposit");
            //cmdRR.Parameters.AddWithValue("conditionv", " ('" + alttime + "' > m_rent.start_duration)  AND ('" + alttime + "' <= m_rent.end_duration ) AND m_room.roomno = '" + int.Parse(txtRoomNo.Text) + "' AND  m_room.build_id = '" + Convert.ToInt32(cmbBuilding.SelectedValue) + "'  AND  room_cat_id = m_rent.room_category ");
            ...
            if (p == 1)
            {
                ...
                cmdDis.Parameters.AddWithValue("conditionv", "mr.build_id=msb.build_id AND mr.roomstatus='1'  AND  mr.rowstatus!='2' AND rc.room_cat_id=mr.room_cat_id AND rt.room_category = mr.room_cat_id  AND ( '" + alttime + "' > rt.start_duration)  AND ('" + alttime + "' <= rt.end_duration ) AND  rt.rent >= " + ren + "  AND rt.reservation_type = '3' AND mr.build_id='" + Convert.ToInt32(cmbaltbulilding.SelectedValue) + "' GROUP BY mr.room_id");
            ...
            else
            {
                ...
                cmdDis.Parameters.AddWithValue("tblname", " m_sub_room_category rc, m_sub_building msb ,m_room mr ,m_rent rt");
                cmdDis.Parameters.AddWithValue("attribute", "mr.roomno,mr.room_id");
                cmdDis.Parameters.AddWithValue("conditionv", "mr.build_id=msb.build_id AND mr.roomstatus='1'  AND  mr.rowstatus!='2' AND rc.room_cat_id=mr.room_cat_id AND rt.room_category = mr.room_cat_id  AND ( '" + alttime + "' > rt.start_duration)  AND ('" + alttime + "' <= rt.end_duration ) AND  rt.rent >= " + ren + "  AND rt.reservation_type = '3' AND mr.build_id='" + Convert.ToInt32(cmbaltbulilding.SelectedValue) + "' and mr.housekeepstatus=" + 1 + "  GROUP BY mr.room_id");
                OdbcDataReader drr = objcls.SpGetReader("CALL selectcond(?,?,?)", cmdDis);
                DataTable dtt = new DataTable();
                dtt = objcls.GetTable(drr);
                DataRow row = dtt.NewRow();
    ...
    protected void btnYes_Click(object sender, EventArgs e)
    {
        if (ViewState["action"].ToString() == "Allocate")
        {
            ...
            try
            {
                ...
                string eee = @"Select adv_recieptno from t_roomallocation where adv_recieptno=" + int.Parse(txtreceiptno1.Text) + " and is_plainprint='" + RecOld + "' AND alloc_type='TDB Allocation'";
    ...
    protected void txtReserveNo_TextChanged(object sender, EventArgs e)
    {
        if (txtReserveNo.Text != "")
        {
            ...
            if (dtreserve.Rows.Count > 0)
            {
                ...
                DateTime chkin = Convert.ToDateTime(dtreserve.Rows[0]["chkin"].ToString());
                DateTime chkout = Convert.ToDateTime(dtreserve.Rows[0]["chkout"].ToString());
...
FROM m_rent,
  m_room
WHERE (" + max_check + " > m_rent.start_duration) AND (" + max_check + " <= m_rent.end_duration) AND m_room.room_id = " + dtreserve.Rows[0]["room_id"].ToString() + " AND m_room.build_id = " + dtreserve.Rows[0]["build_id"].ToString() + " AND m_room.room_cat_id = m_rent.room_category  AND m_rent.reservation_type = '3'";
